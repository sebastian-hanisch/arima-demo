"""Rolling-Origin-Auswertung: ARIMA-Modelle, Glättungsmodelle (Stück 2) und das Wochenmittel (Stück 1) auf denselben Ursprüngen.

Ein Ursprung t heißt: bekannt sind die Tage 0..t-1, prognostiziert werden die Tage t..t+h-1. Alle Parameter stammen aus den Trainingstagen (vor dem ersten Ursprung); Zustände und Fehler laufen mit jedem neuen Tag weiter."""
...
def arima_forecast(model_filt, org, h, bias_correct=True):
    model, filt = model_filt
    return SAR.forecast_origins(model, filt, org, h, bias_correct)


def ets_forecast(model_states, org, h):
    model, states = model_states
    return ETS.forecast_origins(model, states, org, h)
